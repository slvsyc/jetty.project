...
public class AttributeNormalizerTest
{
    ...
    public void testNormalizeJettyHomeAsFile(final Scenario scenario)
    {
        ...
        assertNormalize(scenario, new File(scenario.jettyHome), "${jetty.home}");
    }

    @ParameterizedTest
    @MethodSource("scenarios")
    public void testNormalizeJettyBaseAsURI(final Scenario scenario)
    {
        // Normalize jetty.base as URI path
        assertNormalize(scenario, new File(scenario.jettyBase).toURI(), "${jetty.base.uri}");
    }

    @ParameterizedTest
    @MethodSource("scenarios")
    public void testNormalizeJettyHomeAsURI(final Scenario scenario)
    {
        // Normalize jetty.home as URI path
        assertNormalize(scenario, new File(scenario.jettyHome).toURI(), "${jetty.home.uri}");
    }
